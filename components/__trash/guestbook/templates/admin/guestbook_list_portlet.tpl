<div class="portlet-body form">
	<form action="" class="form-horizontal validate" method="post" enctype="multipart/form-data">
		...
		<div class="control-group">
			<label class="control-label">Текст описания на странице</label>
			<div class="controls">
				<textarea class="span12 m-wrap ckeditor" name="text" rows="8">{$aRequest.text}</textarea>
			</div>
		</div>
...
<table class="table table-striped table-bordered table-advance table-hover" id="table1">
	...
	<tbody>
		{foreach from=$aGuestbook item=item name=childs}
			<tr>
				<td>{$item->getId()}</td>
				...
				<td>
					<a href="{$aTemplate.node_url}edit/{$item->getId()}/">{$item->getAuthor()}</a>
				</td>
				<td>
					{$item->getDatecreate()|date_format}
				</td>
